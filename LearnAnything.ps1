function Find {
    ...
    if(!(Test-Path $username) -and !(Test-Path $password) -and !(Test-Path $emailaddress) -and !(Test-Path $apikey)){
        Get-Content "$PSScriptRoot\utils\username.txt" | ConvertTo-SecureString -AsPlainText -Force  | Export-Clixml -Path ".\utils\username.xml"
        Get-Content "$PSScriptRoot\utils\password.txt" | ConvertTo-SecureString -AsPlainText -Force | Export-Clixml -Path ".\utils\password.xml"
        Get-Content "$PSScriptRoot\utils\emailaddress.txt" | ConvertTo-SecureString -AsPlainText -Force  | Export-Clixml -Path ".\utils\emailaddress.xml"
        Get-Content "$PSScriptRoot\utils\apikey.txt" | ConvertTo-SecureString -AsPlainText -Force  | Export-Clixml -Path ".\utils\apikey.xml"
